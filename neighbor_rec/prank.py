...
class PersonalRank(object):
    ...
    def predict(self, user_id, top_n=10):
        if not os.path.exists('./data/prank/prank_{}.neighbor_rec'.format(user_id)):
            self.train(user_id)
        self.load(user_id)
    ...
    def save(self, user_id):
        f = open('./data/prank/prank_{}.neighbor_rec'.format(user_id), 'wb')
        pickle.dump(self.params, f)
        f.close()

    def load(self, user_id):
        f = open('./data/prank/prank_{}.neighbor_rec'.format(user_id), 'rb')
